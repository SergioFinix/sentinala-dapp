...
export function useTraderVaults() {
  const { address } = useAccount();

  // Get vault IDs for the trader
  const { data: vaultIds, isLoading: isLoadingIds, refetch: refetchIds, error } = useReadContract({
    address: CONTRACT_ADDRESSES.VAULT_FACTORY as `0x${string}`,
    abi: VAULT_FACTORY_ABI,
    functionName: 'getVaultsByTrader',
  ...
  // Debug logging
  if (address) {
    console.log('🔍 useTraderVaults Debug:', {
      address,
